...
class NotificationWebhooksApi:
    ...
    def notification_webhooks_list(
        ...
        status: Annotated[Optional[StrictStr], Field(description="Status of the incoming webhook  * `active` - Active * `active_tmp` - Active but 5xx (or similar) error detected * `inactive_tmp` - Temporary inactive because of 5xx (or similar) error * `inactive_permanent` - Permanently inactive")] = None,
