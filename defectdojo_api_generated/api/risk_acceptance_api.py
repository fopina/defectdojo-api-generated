...
class RiskAcceptanceApi:
    ...
    def risk_acceptance_list(
        ...
        decision: Annotated[Optional[StrictStr], Field(description="Risk treatment decision by risk owner  * `A` - Accept (The risk is acknowledged, yet remains) * `V` - Avoid (Do not engage with whatever creates the risk) * `M` - Mitigate (The risk still exists, yet compensating controls make it less of a threat) * `F` - Fix (The risk is eradicated) * `T` - Transfer (The risk is transferred to a 3rd party)")] = None,
        ...
        o: Annotated[Optional[List[StrictStr]], Field(description="Ordering  * `name` - Name * `-name` - Name (descending)")] = None,
        offset: Annotated[Optional[StrictInt], Field(description="The initial index from which to return the results.")] = None,
        owner: Optional[StrictInt] = None,
        reactivate_expired: Optional[StrictBool] = None,
        recommendation: Annotated[Optional[StrictStr], Field(description="Recommendation from the security team.  * `A` - Accept (The risk is acknowledged, yet remains) * `V` - Avoid (Do not engage with whatever creates the risk) * `M` - Mitigate (The risk still exists, yet compensating controls make it less of a threat) * `F` - Fix (The risk is eradicated) * `T` - Transfer (The risk is transferred to a 3rd party)")] = None,
        ...
        if expiration_date_handled is not None:
            if isinstance(expiration_date_handled, datetime):
                _query_params.append(
                    (
                        'expiration_date_handled',
                        expiration_date_handled.strftime(
                            self.api_client.configuration.datetime_format
                        )
                    )
                )
        ...
        if expiration_date_warned is not None:
            if isinstance(expiration_date_warned, datetime):
                _query_params.append(
                    (
                        'expiration_date_warned',
                        expiration_date_warned.strftime(
                            self.api_client.configuration.datetime_format
                        )
                    )
                )
