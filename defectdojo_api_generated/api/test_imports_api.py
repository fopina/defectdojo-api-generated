...
class TestImportsApi:
    ...
    def test_imports_list(
        ...
        limit: Annotated[Optional[StrictInt], Field(description="Number of results to return per page.")] = None,
        o: Annotated[Optional[List[StrictStr]], Field(description="Ordering  * `id` - Id * `-id` - Id (descending) * `created` - Created * `-created` - Created (descending) * `modified` - Modified * `-modified` - Modified (descending) * `version` - Version * `-version` - Version (descending) * `branch_tag` - Branch tag * `-branch_tag` - Branch tag (descending) * `build_id` - Build id * `-build_id` - Build id (descending) * `commit_hash` - Commit hash * `-commit_hash` - Commit hash (descending)")] = None,
        offset: Annotated[Optional[StrictInt], Field(description="The initial index from which to return the results.")] = None,
        test: Optional[StrictInt] = None,
        test_import_finding_action__action: Annotated[Optional[StrictStr], Field(description="* `N` - created * `C` - closed * `R` - reactivated * `U` - left untouched")] = None,
        ...
        if test_import_finding_action__created is not None:
            if isinstance(test_import_finding_action__created, datetime):
                _query_params.append(
                    (
                        'test_import_finding_action__created',
                        test_import_finding_action__created.strftime(
