...
class Finding(BaseModel):
    ...
    cvssv3: Optional[Annotated[str, Field(strict=True, max_length=117)]] = Field(default=None, description="Common Vulnerability Scoring System version 3 (CVSSv3) score associated with this flaw.")
    ...
    severity: Annotated[str, Field(strict=True, max_length=200)] = Field(description="The severity level of this flaw (Critical, High, Medium, Low, Info).")
    ...
    verified: Optional[StrictBool] = Field(default=None, description="Denotes if this flaw has been manually verified by the tester.")
    ...
    last_status_update: Optional[datetime] = Field(description="Timestamp of latest status update (change in status related fields).")
    ...
    thread_id: StrictInt
    mitigated: Optional[datetime] = Field(description="Denotes if this flaw has been fixed by storing the date it was fixed.")
    numerical_severity: Annotated[str, Field(strict=True, max_length=4)] = Field(description="The numerical representation of the severity (S0, S1, S2, S3, S4).")
    ...
    payload: Optional[StrictStr] = Field(description="Payload used to attack the service / application and trigger the bug / problem.")
    hash_code: Optional[StrictStr] = Field(description="A hash over a configurable set of fields that is used for findings deduplication.")
    ...
    scanner_confidence: Optional[StrictInt] = Field(description="Confidence level of vulnerability which is supplied by the scanner.")
    ...
    duplicate_finding: Optional[StrictInt] = Field(description="Link to the original finding if this finding is a duplicate.")
    ...
    endpoints: List[StrictInt] = Field(description="The hosts within the product that are susceptible to this flaw. + The status of the endpoint associated with this flaw (Vulnerable, Mitigated, ...).")
    ...
    __properties: ClassVar[List[str]] = ["id", "tags", "request_response", "accepted_risks", "push_to_jira", "age", "sla_days_remaining", "finding_meta", "related_fields", "jira_creation", "jira_change", "display_status", "finding_groups", "vulnerability_ids", "reporter", "title", "date", "sla_start_date", "sla_expiration_date", "cwe", "epss_score", "epss_percentile", "cvssv3", "cvssv3_score", "url", "severity", "description", "mitigation", "impact", "steps_to_reproduce", "severity_justification", "references", "active", "verified", "false_p", "duplicate", "out_of_scope", "risk_accepted", "under_review", "last_status_update", "under_defect_review", "is_mitigated", "thread_id", "mitigated", "numerical_severity", "last_reviewed", "param", "payload", "hash_code", "line", "file_path", "component_name", "component_version", "static_finding", "dynamic_finding", "created", "scanner_confidence", "unique_id_from_tool", "vuln_id_from_tool", "sast_source_object", "sast_sink_object", "sast_source_line", "sast_source_file_path", "nb_occurences", "publish_date", "service", "planned_remediation_date", "planned_remediation_version", "effort_for_fixing", "test", "duplicate_finding", "review_requested_by", "defect_review_requested_by", "mitigated_by", "last_reviewed_by", "sonarqube_issue", "endpoints", "reviewers", "notes", "files", "found_by", "prefetch"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "tags": obj.get("tags"),
            "request_response": BurpRawRequestResponse.from_dict(obj["request_response"]) if obj.get("request_response") is not None else None,
            "accepted_risks": [RiskAcceptance.from_dict(_item) for _item in obj["accepted_risks"]] if obj.get("accepted_risks") is not None else None,
            ...
            "finding_meta": [FindingMeta.from_dict(_item) for _item in obj["finding_meta"]] if obj.get("finding_meta") is not None else None,
            "related_fields": FindingRelatedFields.from_dict(obj["related_fields"]) if obj.get("related_fields") is not None else None,
            "jira_creation": obj.get("jira_creation"),
            "jira_change": obj.get("jira_change"),
            "display_status": obj.get("display_status"),
            "finding_groups": [FindingGroup.from_dict(_item) for _item in obj["finding_groups"]] if obj.get("finding_groups") is not None else None,
            "vulnerability_ids": [VulnerabilityId.from_dict(_item) for _item in obj["vulnerability_ids"]] if obj.get("vulnerability_ids") is not None else None,
