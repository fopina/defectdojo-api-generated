...
class ImportScan(BaseModel):
    ...
    file: Optional[Annotated[str, Field(strict=True)]] = None
    product_type_name: Optional[StrictStr] = Field(default=None, description="Also referred to as 'Organization' name.")
    product_name: Optional[StrictStr] = Field(default=None, description="Also referred to as 'Asset' name.")
    ...
    group_by: Optional[StrictStr] = Field(
        default=None,
        description='Choose an option to automatically group new findings by the chosen option.  * `component_name` - Component Name * `component_name+component_version` - Component Name + Version * `file_path` - File path * `finding_title` - Finding Title * `vuln_id_from_tool` - Vulnerability ID from Tool',
    )
    ...
    product_id: Optional[StrictInt] = Field(default=None, description="Also referred to as 'Asset' ID.")
    product_type_id: Optional[StrictInt] = Field(default=None, description="Also referred to as 'Organization' ID.")
    statistics: Optional[ImportStatistics] = None
    pro: Optional[List[Any]] = None
    ...
    close_old_findings: Optional[StrictBool] = Field(
        default=False,
        description='Old findings no longer present in the new report get closed as mitigated when importing. If service has been set, only the findings for this service will be closed; if no service is set, only findings without a service will be closed. This only affects findings within the same engagement.',
    )
    close_old_findings_product_scope: Optional[StrictBool] = Field(
        default=False,
        description='Old findings no longer present in the new report get closed as mitigated when importing. If service has been set, only the findings for this service will be closed; if no service is set, only findings without a service will be closed. This only affects findings within the same product.By default, it is false meaning that only old findings of the same type in the engagement are in scope.',
    )
    ...
    def file_validate_regular_expression(cls, value):
        ...
        if not re.match(r'(?:xml|csv|nessus|json|jsonl|html|js|zip|xlsx|txt|sarif|fpr|md|log|fvdl)$', value):
            raise ValueError(
                r'must validate the regular expression /(?:xml|csv|nessus|json|jsonl|html|js|zip|xlsx|txt|sarif|fpr|md|log|fvdl)$/'
            )
        return value
    ...
    def group_by_validate_enum(cls, value):
        ...
        if value not in set(
            ['component_name', 'component_name+component_version', 'file_path', 'finding_title', 'vuln_id_from_tool']
        ):
            raise ValueError(
                "must be one of enum values ('component_name', 'component_name+component_version', 'file_path', 'finding_title', 'vuln_id_from_tool')"
            )
        return value
