...
class ImportStatisticsRequest(BaseModel):
    ...
    """ # noqa: E501
    before: Optional[SeverityStatusStatisticsRequest] = Field(default=None, description="Finding statistics as stored in Defect Dojo before the import")
    delta: Optional[DeltaStatisticsRequest] = Field(default=None, description="Finding statistics of modifications made by the reimport. Only available when TRACK_IMPORT_HISTORY has not been disabled.")
    after: SeverityStatusStatisticsRequest = Field(description="Finding statistics as stored in Defect Dojo after the import")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "before": SeverityStatusStatisticsRequest.from_dict(obj["before"]) if obj.get("before") is not None else None,
            "delta": DeltaStatisticsRequest.from_dict(obj["delta"]) if obj.get("delta") is not None else None,
            "after": SeverityStatusStatisticsRequest.from_dict(obj["after"]) if obj.get("after") is not None else None
        })
        return _obj
