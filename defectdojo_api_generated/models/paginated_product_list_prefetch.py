...
class PaginatedProductListPrefetch(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["authorization_groups", "members", "prod_type", "product_manager", "regulations", "sla_configuration", "team_manager", "technical_contact"]
        ...
        if self.authorization_groups:
            for _key_authorization_groups in self.authorization_groups:
                if self.authorization_groups[_key_authorization_groups]:
                    _field_dict[_key_authorization_groups] = self.authorization_groups[_key_authorization_groups].to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "authorization_groups": dict(
                (_k, DojoGroup.from_dict(_v))
                for _k, _v in obj["authorization_groups"].items()
            )
            ...
            "prod_type": dict(
                (_k, ProductType.from_dict(_v))
                for _k, _v in obj["prod_type"].items()
            )
            if obj.get("prod_type") is not None
            else None,
            "product_manager": dict(
                (_k, UserStub.from_dict(_v))
                for _k, _v in obj["product_manager"].items()
            )
            if obj.get("product_manager") is not None
            else None,
            "regulations": dict(
                (_k, Regulation.from_dict(_v))
                for _k, _v in obj["regulations"].items()
            )
            if obj.get("regulations") is not None
            else None,
            "sla_configuration": dict(
                (_k, SLAConfiguration.from_dict(_v))
                for _k, _v in obj["sla_configuration"].items()
            )
            if obj.get("sla_configuration") is not None
            else None,
            "team_manager": dict(
                (_k, UserStub.from_dict(_v))
                for _k, _v in obj["team_manager"].items()
            )
            if obj.get("team_manager") is not None
            else None,
            "technical_contact": dict(
                (_k, UserStub.from_dict(_v))
                for _k, _v in obj["technical_contact"].items()
            )
