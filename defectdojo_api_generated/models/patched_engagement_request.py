...
class PatchedEngagementRequest(BaseModel):
    ...
    version: Optional[Annotated[str, Field(strict=True, max_length=100)]] = Field(default=None, description="Version of the product the engagement tested.")
    ...
    tracker: Optional[Annotated[str, Field(strict=True, max_length=200)]] = Field(default=None, description="Link to epic or ticket system with changes to version.")
    ...
    status: Optional[StrictStr] = Field(default=None, description="* `Not Started` - Not Started * `Blocked` - Blocked * `Cancelled` - Cancelled * `Completed` - Completed * `In Progress` - In Progress * `On Hold` - On Hold * `Waiting for Resource` - Waiting for Resource")
    engagement_type: Optional[StrictStr] = Field(default=None, description="* `Interactive` - Interactive * `CI/CD` - CI/CD")
    build_id: Optional[Annotated[str, Field(strict=True, max_length=150)]] = Field(default=None, description="Build ID of the product the engagement tested.")
    commit_hash: Optional[Annotated[str, Field(strict=True, max_length=150)]] = Field(default=None, description="Commit hash from repo")
    branch_tag: Optional[Annotated[str, Field(strict=True, max_length=150)]] = Field(default=None, description="Tag or branch of the product the engagement tested.")
    ...
    deduplication_on_engagement: Optional[StrictBool] = Field(default=None, description="If enabled deduplication will only mark a finding in this engagement as duplicate of another finding if both findings are in this engagement. If disabled, deduplication is on the product level.")
    ...
    source_code_management_server: Optional[StrictInt] = Field(default=None, description="Source code server for CI/CD test")
    orchestration_engine: Optional[StrictInt] = Field(default=None, description="Orchestration service responsible for CI/CD test")
    __properties: ClassVar[List[str]] = ["tags", "name", "description", "version", "first_contacted", "target_start", "target_end", "reason", "tracker", "test_strategy", "threat_model", "api_test", "pen_test", "check_list", "status", "engagement_type", "build_id", "commit_hash", "branch_tag", "source_code_management_uri", "deduplication_on_engagement", "lead", "requester", "preset", "report_type", "product", "build_server", "source_code_management_server", "orchestration_engine"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['Not Started', 'Blocked', 'Cancelled', 'Completed', 'In Progress', 'On Hold', 'Waiting for Resource']):
            raise ValueError("must be one of enum values ('Not Started', 'Blocked', 'Cancelled', 'Completed', 'In Progress', 'On Hold', 'Waiting for Resource')")
        return value
